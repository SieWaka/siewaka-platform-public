...
export const SamplesTab = ({
  ...
  const sampleToEdit =
    typeof router.query.sample === 'string'
      ? parseInt(router.query.sample)
      : undefined;
  const sampleToEditData = samples?.find(
    (sample) => sample.id === sampleToEdit
  );
          ...
          isAbleToPerformActions ? (
            <Link href={`${samplingPoint.id}?addSample=true`}>
              <Button
                ...
                iconSize="xxs"
                spanClassName={'hidden lg:block text-primary'}
                icon={<PlusIcon />}
              >
                Nueva muestra
              </Button>
