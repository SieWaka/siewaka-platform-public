...
export const SamplingPointDetailsTable: React.FC<
  SamplingPointDetailsTableProps
> = ({ samples, devices, samplingPointId, isAbleToPerformActions }) => {
  ...
  const transformedSamples: TransformedSampleType[] | undefined = samples?.map(
    (sample) => ({
  ...
  const transformedDevices: TransformedDeviceType[] | undefined = devices?.map(
    (device) => ({
      name: device.name,
      id: device.id,
  ...
  const confirmDeleteSample = async () => {
    ...
    try {
      await axios.delete(
        `/api/sampling-points/${samplingPointId}/samples/${sampleId}`
      );
      ...
      {transformedSamples?.length ? (
        <Table
          ...
          actions={[
            ({ data }) =>
              isAbleToPerformActions ? (
                <IconButton
                  onClick={() =>
                    handleDeleteSample((data as TransformedSampleType).id)
                  }
                  icon={<TrashIcon />}
            ...
            ({ data }) =>
              isAbleToPerformActions ? (
                <Link
                  href={`${samplingPointId}?sample=${
                    (data as TransformedSampleType).id
                  }`}
                >
                  <IconButton
                    icon={<IconPencil />}
          ...
          formatCell={[
            {
              condition: (key) =>
                key === 'date' || key === 'time' || key === 'responsible',
              component: ({ value }) => {
            ...
            },
            {
              ...
              component: ({ value }) => {
                return (
                  <Text as="p2" className="text-center">
                    {value}
                  </Text>
                );
              },
          ...
          formatCell={[
            {
              ...
              component: ({ value }) => {
                return (
                  <Text
                    as="p2"
                    className="text-left text-ellipsis truncate w-full overflow-hidden"
                  >
                    {value}
                  </Text>
                );
