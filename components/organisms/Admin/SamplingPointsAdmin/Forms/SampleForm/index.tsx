...
export const SampleForm: React.FC<SampleOnSamplingPointFormProps> = ({
  ...
  const userDevices = samplingPoint.devices?.filter(
    (device) => device.owner.id === user?.id
  );
  const title = `${isAdding ? 'Agregar' : 'Editar'} muestra`;
  const owner = isEditing
    ? `${sample?.takenBy.firstName} ${sample?.takenBy.lastName}`
    : `${user?.firstName} ${user?.lastName}`;

  const initialData = {
    samplingPointId: samplingPoint.id,
    date: isEditing
      ? moment(sample?.takenAt).format('YYYY-MM-DD') ||
        moment().format('YYYY-MM-DD')
      : moment().format('YYYY-MM-DD').toString(),
    time: isEditing
      ? moment(sample?.takenAt).format('HH:mm') || moment().format('HH:mm')
      : moment().format('HH:mm'),
  ...
  useEffect(() => {
    const formattedMeasurementValues =
      Object.keys(initialData.measurementValues).length !== 0
        ? Object.fromEntries(
            Object.entries(initialData.measurementValues).map(
              ([parameterId, value]) => [parameterId, !isNaN(value) ? value : 0]
            )
          )
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const numericValue = ['latitude', 'longitude'].includes(name)
      ? parseFloat(value)
      : value;
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    if (Object.keys(errors).length === 0 || isValidationDone) {
      try {
        ...
        const formattedTakenAt = moment(
          `${date} ${time}`,
          'YYYY-MM-DD HH:mm'
        ).toDate();

        const samples = Array.isArray(samplingPoint.samples)
          ? samplingPoint.samples
          : [];
        const editingSample = sample
          ? samples.find((s) => s.id === sample.id)
          : null;
        const updatedSamples = samples.map((s) =>
          s.id === sample?.id
            ? { ...restData, id: s.id }
            : { ...s, measurementValues: { ...s.measurementValues } }
        );
        const updatedSample = updatedSamples.find(
        ...
        if (isAdding) {
          ...
          await axios.put(
            `/api/sampling-points/${samplingPoint.id}/samples/${editingSample.id}`,
            { ...updatedSample, takenAt: formattedTakenAt }
          );
        ...
        console.error(error);
        window.alert(
          `Hubo un error al ${isAdding ? 'agregar' : 'editar'} la muestra`
        );
      }
    }
  };
  ...
  const renderMeasurementInputs = () => {
    const measurementValues = isEditing
        ...
        {sortedEntries.map(([parameterId, value]) => {
          return (
            <NumberInput
              ...
              defaultValue={value}
              onChange={(e) =>
                handleMeasurementValueChange(parameterId, e.target.value)
              }
  ...
  const confirmDeleteSample = async () => {
    ...
    try {
      await axios.delete(
        `/api/sampling-points/${samplingPoint.id}/samples/${sample?.id}`
      );
      ...
      <FormContainer className="flex flex-col space-y-3 w-full">
        ...
        <div className="flex space-x-4">
          <div className="w-full space-y-2">
            ...
            {!isDistanceValid && (
              <Text as="p3" className="ml-2 text-danger">
                La latitud y longitud no deben superar una distancia de 5 metros
                del punto principal.
              </Text>
            )}
            {errors.latitude && (
        ...
        <Select
          ...
          value={data.deviceId}
          onChange={(value) =>
            handleSelectChange('deviceId', parseFloat(value as string))
          }
          placeholder={'Módulo'}
          variant={'admin'}
          disabled={!userDevices.length}
        ...
        {!userDevices.length && (
          <Text as="p3" className="ml-2 text-danger">
            Usted no es dueño de ningún módulo relacionado a este punto de toma.{' '}
            <b>No será posible cargar la muestra</b>{' '}
          </Text>
        )}
        {errors.deviceId && (
          <Text as="p3" className="ml-2 text-danger">
