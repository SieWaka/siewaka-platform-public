...
const newsPostSchema = z.object({
  title: z
    ...
    .max(100, 'El título puede tener un máximo de 100 caracteres'),
  description: z
    .string()
    .min(3, 'La descripción es requerida')
    .max(280, 'La descripción puede tener un máximo de 280 caracteres'),
  startDate: z.date(),
  endDate: z.date(),
...
const NewsPostForm: React.FC<NewsPostFormProps> = ({
  ...
  const initialData: Data = {
    ...
    update: {
      ...
      startDate:
        (foundNewsPost?.startDate && new Date(foundNewsPost.startDate)) ||
        new Date(),
      endDate:
        (foundNewsPost?.endDate && new Date(foundNewsPost.endDate)) ||
        new Date(),
