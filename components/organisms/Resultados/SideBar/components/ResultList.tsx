          ...
          ? results.map((result) => {
              return (
                <PreviewCard
                  key={result.id}
                  title={result.name}
                  date={moment(result.lastSample?.takenAt)
                    .utc()
                    .format('DD/MM/yyyy')}
                  tags={capitalize(result.areaType)}
                  items={formatMeasurementValues(
                    result.lastSample?.measurementValues
                  )}
      ...
      {!loading && results && !results.length ? (
        <div className="mt-4 p-6 lg:p-0 mx-3 lg:ml-2 text-center lg:mx-auto ">
          <Text as="p1" className="lg:w-[20rem] w-full lg:mx-auto">
            No se encontraron puntos de toma. <br />
            <Text as="span2">
              Probá cambiar algun filtro o ajustar la zona de búsqueda.
            </Text>
          </Text>
        </div>
      ) : (
