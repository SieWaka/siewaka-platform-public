...
export const Item: React.FC<ItemProps> = (props) => {
  const { sampleParameters, date, location, distance, description, owner } =
    props;

  const sampleTypes =
    sampleParameters &&
    Object.keys(sampleParameters).map((sample, index) => {
        ...
        <div className="my-3 ml-2 items-center w-full">
          {sampleTypes ? (
            sampleTypes.map((sample) => (
              ...
              >
                <Icon size="small" icon={sample.icon} className="!m-0 !p-0" />

                <div>
                  <Text as="p2" className="ml-2 py-0 font-semibold text-sm">
                    {capitalize(sample.name.split(' ')[0])}{' '}
                    <Text as="span2" className="!font-normal">
                      {sample.value}
                    </Text>
                  </Text>
                </div>
          ...
          {date != '' && (
            <Pill type="tertiary" className={'inline-block my-4'}>
              {date}
            </Pill>
          )}
          ...
          {location && (
            <div className="my-2 ">
              <Text
                as="p2"
                className="hidden lg:block text-dark-gray !font-normal"
              >
                Ubicación
              </Text>
    ...
    >
      <div className="max-h-full lg:max-h-[calc(100vh_-_135px)] bg-white py-8 px-4 rounded-2xl overflow-auto scroll-style space-y-2">
        <div className="w-full flex justify-center lg:justify-between">
          ...
          <button className={'w-5 text-secondary flex'} onClick={onClose}>
            <XMarkIcon />
          </button>
        </div>

        <Tabs
          content={content as ContentProp[]}
