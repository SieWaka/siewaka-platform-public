            ...
            items: [
              {
                ...
                date: moment(activeResult.lastSample?.takenAt)
                  .utc()
                  .format('DD/MM/yyyy'),
                location: `${activeResult.latitude}, ${activeResult.longitude}`,
                distance: activeResult.distance
                  ? `a ${activeResult.distance}`
                  : '',
                description: activeResult.description ?? '',
                owner: activeResult.owner?.organizationName ?? 'Sie Waka',
              },
            ],
          },
          {
...
const PreviewModal = ({
  ...
  return (
    <div
      className={`${
        mapVisibility == 'hidden' ? 'hidden' : 'block'
    ...
    >
      ...
      <PreviewCard
        ...
        items={formatMeasurementValues(
          activeResult.lastSample?.measurementValues
        )}
        onClick={() =>
          setDetailModes((prevModes) => ({
            ...prevModes,
        ...
        content={[
          {
            ...
            items: [
              {
                sampleParameters: activeResult.lastSample?.measurementValues,
                date: activeResult.lastSample
                  ? moment(activeResult.lastSample?.takenAt)
                      .utc()
                      .format('DD/MM/yyyy')
                  : '',
                location: `${activeResult.latitude}, ${activeResult.longitude}`,
...
const Modal = (props: ModalProps) => {
  ...
  return (
    <div>
      {detailModes.showFullDetail && <FullDetailModal {...props} />}
      {detailModes.showPreview && <PreviewModal {...props} />}
    </div>
  );
};
