...
async function main() {
  ...
  const mexicoCountry = await prisma.country.findUniqueOrThrow({
    where: { name: 'México' },
  });

  await prisma.faq.create({
    data: {
      question: '¿Cómo funciona Sie Waka?',
  ...
  const device = await prisma.device.create({
    data: {
      ...
      components:
        'ESP32 / Gravity Analog pH Sensor/ Gravity Analog Electrical Conductivity Sensor / DS18B20',
      samplingPoint: {
        connect: {
          id: samplingPoint.id,
        },
  ...
  for (let i = 0; i < 5; i++) {
    await prismaClient.cRQuestion.create({
      data: {
        priority: i,
        hidden: false,
        question: `que tan sucia esta tu agua ${i}?`,
      },
    });
  }
}
