...
interface AuxGetDeviceResponseInterface
  extends Pick<
    Device,
    'id' | 'name' | 'samplingPointId' | 'description' | 'components'
  > {
  owner: Pick<
    User,
    'id' | 'firstName' | 'lastName' | 'organizationName' | 'email'
  >;
...
interface AuxGetDevicesResponseInterface
  extends Pick<Device, 'id' | 'name' | 'description'> {
  owner: Pick<User, 'organizationName'>;
}

export type GetDevicesResponse = AuxGetDevicesResponseInterface[];

export type GetDevicesResponseForTable = (Omit<
  AuxGetDevicesResponseInterface,
  'owner'
> & {
  organizationName: AuxGetDevicesResponseInterface['owner']['organizationName'];
})[];
