...
const handler: NextApiHandler = async (req, res) => {
  ...
  if (process.env.USE_MAPBOX_DISTANCE === 'true') {
    const mapBoxResponse = await axios.get(
      `https://api.mapbox.com/directions-matrix/v1/mapbox/walking/${queryData.coords.lng},${queryData.coords.lat};${samplingPoint.longitude},${samplingPoint.latitude}?access_token=${process.env.MAPBOX_TOKEN}&sources=0&annotations=distance`
    );
    ...
    const googleMapsResponse = await axios.get(
      `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${queryData.coords.lat},${queryData.coords.lng}&destinations=${samplingPoint.latitude},${samplingPoint.longitude}&key=${process.env.GOOGLE_MAPS_API_KEY}&mode=walking`
    );
    // Google Maps already formats the distance.
    distance =
      googleMapsResponse.data?.rows[0]?.elements[0]?.distance?.text ?? null;
