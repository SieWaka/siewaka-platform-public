...
async function getHandler(
  ...
  try {
    const device: GetDeviceResponse | null =
      await prismaClient.device.findUnique({
        where: {
          id: id,
        },
        select: {
...
async function putHandler(
  ...
  try {
    const oldDevice: Device | null = await prismaClient.device.findUnique({
      where: { id: id },
    });
    ...
    let samplingPointUpdate:
      | Prisma.SamplingPointUpdateOneWithoutDevicesNestedInput
      | undefined;
...
async function deleteHandler(
  ...
  try {
    const device: Device | null = await prismaClient.device.findUnique({
      where: { id: id },
    });
    ...
    const deletedDevice = await prismaClient.device.delete({
      where: { id: id },
    });
