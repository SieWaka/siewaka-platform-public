...
const handler: NextApiHandler = async (req, res) => {
  ...
  if (!device) {
    return res
      .status(404)
      .json({ error: `Device with id ${req.body.deviceId} not found` });
  }

  if (!device.apiKey) {
    return res.status(403).json({
      error: `Device with id ${req.body.deviceId} does not have its api key set`,
    });
  }
  ...
  if (!device.samplingPointId) {
    return res.status(400).json({
      error: `Device with id ${req.body.deviceId} does not have an associated sampling point`,
    });
  }

  const sample = await createSample(
    device.id,
    device.samplingPointId,
    device.ownerId,
    body.latitude,
    body.longitude,
