...
const SearchButton = React.memo<SearchButtonProps>(
  (props: SearchButtonProps) => {
    const { enabled, onClick } = props;

    return (
      <Button
        className={'my-5 w-full lg:w-80 lg:mr-1 mb-5 lg:my-5'}
        disabled={!enabled}
        variant={'primary'}
        onClick={onClick}
      >
...
const SearchAllButton = React.memo<SearchAllButtonProps>((props) => {
  ...
  return (
    <Button
      type="button"
      className={
        'w-full lg:ml-1 mb-5 lg:my-5 lg:w-80 flex flex-row aling-center justify-center'
      }
      variant={'secondary'}
      onClick={onClick}
      icon={<Squares2X2Icon />}
...
export const filterList = [
  {
    key: 'Sie Waka',
    name: 'Módulos de Sie Waka',
    category: 'takenByOrganizations',
  },
  {
    key: 'Independiente',
...
const Home: NextPage = React.memo(() => {
  ...
  return (
    ...
    >
      <Head>
        <title>Sie Waka</title>
      </Head>
      <MainContainer
        className={'w-full h-full lg:w-3/5 lg:mx-4 pt-8 mt-12 lg:flex-grow-0'}
      >
        <div className="w-full flex flex-col justify-center">
          <div className="flex-col">
            ...
            <p className={'text-xs text-black text-center my-3'}>
