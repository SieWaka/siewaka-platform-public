...
const setAboutSectionContent = async (id?: string, text?: string) => {
  return await axios.put(`/api/about/${id}`, {
    text,
  });
};
...
const setFaqsSectionContent = async (
  id?: string,
  question?: string,
  answer?: string
) => {
...
// @ts-ignore
const Contenido: NextPage<ContenidoProps> = ({
    ...
    <div ref={faqsRef} className="bg-box-background h-screen">
      <AdminLayout title="Contenido">
        <ul className="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 ">
          <li className="mr-2">
            <a
              ...
              className={`inline-block p-4 rounded-t-lg ${
                activeTab === 'about-us' ? 'active text-white bg-primary' : ''
              }`}
          ...
          <li className="mr-2">
            <a
              href="#faq"
              className={`inline-block p-4 rounded-t-lg ${
                activeTab === 'faq' ? 'active text-white bg-primary' : ''
              }`}
        ...
        <div id="about-us" className={activeTab === 'about-us' ? '' : 'hidden'}>
          <h2 className="text-lg text-black font-bold my-2 mb-8">
            Sobre Sie Waka
          </h2>
          {aboutContent &&
            aboutContent.map((currentContent, index) => (
              <TipTapEditor
                ...
                callback={setAboutSectionContent}
              />
            ))}
        </div>
