...
const dataSchema = z.object({
  ...
  organizationCountryId: z
    .string()
    .uuid('El país de la organización es requerido'),
...
const UserForm: NextPage<ServerSideProps> = ({
  user,
  countries,
    ...
    try {
      await axios.post(
        `/api/admin/users/${user.id}/${
          user.status === 'ACTIVE' ? 'inactivate' : 'aprobar'
        }`
      );
      window.alert('Usuario actualizado');
        ...
        <form className="flex flex-col space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="w-full flex justify-center lg:justify-start py-3 space-x-4">
            <Link href="/admin/users">
              <Button icon={<XMarkIcon />} iconSize="small" variant="secondary">
                Cancelar
              </Button>
            </Link>
            <Button
