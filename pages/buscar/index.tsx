...
const SearchLocation: NextPage<ServerSideProps> = ({
  ...
  return (
    <div
      className={
        'flex flex-wrap flex-grow content-start justify-center bg-main-image bg-white'
      }
    >
      <Head>
        <title>Sie Waka - Buscar por Ubicación</title>
      </Head>
      <MainContainer
        className={'w-full h-full lg:w-3/5 lg:mx-4 mt-4 p-8 lg:flex-grow-0'}
      >
        <div className="w-full flex flex-col justify-center">
          <div
            className={
              'px-content pt-4 flex flex-col text-center lg:flex-row justify-center'
            }
          >
            <p className="text-gray-700 text-xs p-1 mx-3">Estás buscando:</p>
          ...
          <div className="flex-col">
            <Text as="h2" className="text-center my-3">
              ¿En qué lugar?
            </Text>
            <p
              className={
                'w-full text-xs text-black text-center mt-2 mb-3 px-10'
              }
            >
              Podés buscar por ciudad, departamento o barrio. También podés
              buscar por el nombre o la dirección de un centro que ya conozcas.
            </p>
          </div>
          <div className="flex w-full justify-end">
            <GlobeAltIcon className="w-4 text-gray-600 mt-1.5 mr-1" />
            <p className="text-gray-600 text-xs mb-2 mt-4">
              Estas buscando en:{' '}
            </p>
            <button
              onClick={() => setShow(!show)}
              className={'bg-inherit text-gray-800 text-xs ml-2 mb-2 mt-4 mr-4'}
            >
              <div className="mx-1 flex justify-between w-full">
                {countries.map((countryData) => {
                  if (countryData.code == country) {
                    return (
                      <span key={countryData.code}>{countryData.name}</span>
                    );
                  } else {
                    ('');
                  }
