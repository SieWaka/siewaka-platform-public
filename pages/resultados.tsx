...
export const resultWithinBoundaries = (marker: Coordinates, bounds: Bounds) => {
...
const Results: NextPage<ServerSideProps> = ({
  googleMapsApiKey,
  ...
  useEffect(() => {
    ...
    if (coords) {
      ...
      getCurrentLocation(
        (coords) => setMapPosition(getMapPosition(coords)),
        setError
      );
    }
  ...
  const handleMarkerClick = async (resultId: string) => {
    const selectedResultSamples = await axios
      .get(`/api/sampling-points/${resultId}/samples`)
      .then((res) => {
        return res.data;
      });

    const selectedResultDistance = await axios
      .get(
        `/api/sampling-points/distances?coords[lat]=${mapPosition?.coords.lat}&coords[lng]=${mapPosition?.coords.lng}&samplingPointId=${resultId}`
      )
      .then((res) => res.data);
    ...
    const selectedResult: GetSamplingPointResponse = Object.assign(
      ...
      {
        ...(results.find(
          (result: GetSamplingPointResponse) => result.id === resultId
        ) ?? null),
  ...
  return (
    <div className="overflow-hidden lg:overflow-visible">
      <Head>
        <title>Sie Waka - Puntos de toma</title>
      </Head>

      {error !== '' && (
        <div className="w-full flex flex-col items-center">
          <Text as="p1" className="text-center m-6">
            {error}
          </Text>
          <Link href="/">
      ...
      >
        {mapPosition && (
          <div className={'flex bg-gray-50 lg:bg-white'}>
            ...
            <div
              className={`${
                mapVisibility == 'hidden' ? 'relative' : 'absolute'
