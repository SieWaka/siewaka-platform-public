...
const AccordionItem = (props: AccordionItemProps) => {
  ...
  return (
    <div className={'flex flex-col rounded-2xl border-2 w-full my-4'}>
      <button
        className={'py-4 flex flex-row w-full cursor-pointer'}
        onClick={() => setOpen(!open)}
      >
        <QuestionMarkCircleIcon className={'h-6 w-5 text-primary mx-4'} />
        {/* <p className={'text-l font-title text-black font-semibold'}></p> */}
        <div
          className="text-l font-title text-black font-semibold"
          dangerouslySetInnerHTML={handleHtml(title)}
        ></div>
      ...
      {open && (
        <div
          className="px-8 my-4"
          dangerouslySetInnerHTML={handleHtml(children)}
        ></div>
      )}
    </div>
  );
...
export default function FAQs() {
  ...
  return (
    <>
      <Head>
        <title>Sie Waka - Preguntas Frecuentes</title>
      </Head>
